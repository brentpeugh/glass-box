...
export interface ResolvedRows {
  kind: "retention" | "col_sum" | "delta" | "opex";
  [k: string]: any;
}
...
export function createEngine(bundle: Bundle) {
  ...
  function nrr(seg: string | null, startQ: string, endQ: string) {
    ...
    const b = BENCH.nrr;
    return mk(id, "nrr", `${seg ? seg : "Blended"} NRR`, (end / start) * 100, "percent", { op: "ratio_pct", description: "100 * cohort end ARR / cohort start ARR", inputs: [mref(endMv.id), mref(startMv.id)], configRefs: ["benchmark.nrr"] }, { basis: { kind: "benchmark", ref: "nrr", thr: b.threshold, good: "above", delta: (end / start) * 100 - b.threshold } as any });
  }
  function grr(seg: string | null, startQ: string, endQ: string) {
    ...
    const b = BENCH.grr;
    return mk(id, "grr", `${seg ? seg : "Blended"} GRR`, (retained / startMv.value) * 100, "percent", { op: "ratio_pct", description: "100 * retained ARR / cohort start ARR", inputs: [mref(retMv.id), mref(startMv.id)], configRefs: ["benchmark.grr"] }, { basis: { kind: "benchmark", ref: "grr", thr: b.threshold, good: "above", delta: (retained / startMv.value) * 100 - b.threshold } as any });
  }
  ...
  function resolveLeaf(sel: RowSelector): ResolvedRows {
    ...
    const col = sel.col!; const rows = pool.filter((r) => r[col] > 0).map((r) => ({ id: r.customer_id, v: r[col] })).sort((a, b) => b.v - a.v); return { kind: "col_sum", col, n: rows.length, rows };
  }
